...
import StatsCard from '../../../components/common/StatsCard'; // KORJATTU POLKU
import { Cloud, CloudRain, Sun, Wind, Thermometer, BrainCircuit } from 'lucide-react';
import WeatherAnalysis from '../weather/analysis'; // UUSI AIVOKOMPONENTTI
...
export default function WeatherTab({ data }) {
  ...
  // Haetaan uusin analytiikkarivi tekstejä varten
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {/* 1. TEKOÄLYN SEMANTTINEN YHTEENVETO */}
      ...
      {/* 3. UUSI SYVÄANALYYSIMATEMATIIKKA */}
      <WeatherAnalysis historyAnalytics={historyAnalytics} weatherForecast={weatherForecast} />
      ...
      {weatherForecast && weatherForecast.length > 0 ? (
          ...
          {weatherForecast.slice(0, 6).map((forecast, index) => {
            ...
            return (
              <div key={index} className="ui-panel" style={{ padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '12px' }}>
                <span className="text-technical" style={{ fontWeight: 600 }}>klo {formatTime(forecast.target_time)}</span>
                {getWeatherIcon(clouds, precip)}
                <div>
                  <div style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--color-text-main)' }}>{temp}°C</div>
                  ...
                  {precip > 0 && (
                    <div style={{ fontSize: '0.75rem', color: 'var(--color-electric)', marginTop: '2px', fontWeight: 600 }}>{precip.toFixed(1)} mm</div>
                  )}
                </div>
              </div>
            );
        ...
        <div className="ui-panel" style={{ padding: '24px', textAlign: 'center' }}><p className="text-muted">Ei sääennustedataa saatavilla.</p></div>
      )}
