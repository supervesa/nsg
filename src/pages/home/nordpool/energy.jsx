...
export default function NordpoolEnergy({ nordpoolPrices = [] }) {
  ...
  // SUODATUS: Otetaan mukaan vain ne hintajaksot, joiden loppuaika on tulevaisuudessa
  const futurePrices = nordpoolPrices.filter(p => new Date(p.end_time) > now);

  if (!futurePrices || futurePrices.length === 0) {
    return <p className="text-muted">Ei tulevia hintatietoja saatavilla tälle hetkelle.</p>;
  }

  const SLOT_HOURS = 6;
  const CHARGE_POWER_KW = 11; // 11 kW 3-vaihelataus

  // Järjestetään tulevat hinnat suuruusjärjestykseen halvinta/kalleinta varten
  ...
  // Ajan ja päivämäärän formatointi Suomen aikaan
  ...
  // Älykäs ajan esitys (jos sama päivä, näytä päivä vain kerran)
  const formatRangeText = (startIso, endIso) => {
    ...
    if (startDateStr === endDateStr) {
      return `${startDateStr} klo ${startTimeStr} – ${endTimeStr}`;
    }
    return `${startDateStr} klo ${startTimeStr} – ${endDateStr} klo ${endTimeStr}`;
  };

  // OPTIMAALINEN LATAUSIKKUNA (6 TUNTIA) perustuen puhtaasti tuntihintoihin
  ...
  if (chronologicalPrices.length >= SLOT_HOURS) {
    for (let i = 0; i <= chronologicalPrices.length - SLOT_HOURS; i++) {
      ...
      const slotCost = window.reduce((acc, curr) => acc + (parseFloat(curr.price) * CHARGE_POWER_KW / 100), 0);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {/* Päägridi: minmax(280px, 1fr) takaa siistin taittumisen */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
        
        {/* Optimaalinen latausikkuna (vie aina koko leveyden) */}
        {bestSlot ? (
          <div className="ui-panel" style={{ padding: '16px', gridColumn: '1 / -1', borderLeft: '4px solid var(--color-saab)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', color: 'var(--color-text-main)' }}>
              ...
              <h4 style={{ margin: 0, fontSize: '0.875rem', fontWeight: 600 }}>Optimaalinen latausikkuna ({SLOT_HOURS}h peräkkäin, perustuu tuntihintoihin)</h4>
            </div>
            
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', gap: '16px', alignItems: 'center' }}>
              <div>
                <span className="text-label" style={{ marginBottom: '4px' }}>Aikaikkuna</span>
              ...
              <div style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
                <div style={{ textAlign: 'right' }}>
                  <span className="text-label" style={{ marginBottom: '4px' }}>
                    Latauksen kokonaishinta ({CHARGE_POWER_KW} kW)
                  </span>
                  ...
                  <div className="text-technical" style={{ fontSize: '0.75rem', marginTop: '2px' }}>
                    Yhteensä {totalEnergyKwh} kWh tuntihintojen mukaan
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Halvimmat tulevat tunnit (tasakorkuinen flex-laatikko) */}
        <div className="ui-panel" style={{ padding: '16px', boxShadow: 'none', display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          <div style={{ flex: 1 }}>
            {cheapest.length > 0 ? cheapest.map((item, idx) => <PriceRow key={idx} item={item} isExpensive={false} />) : <p className="text-muted">Ei tulevia tunteja.</p>}
          </div>
        </div>

        {/* Kalleimmat tulevat tunnit (tasakorkuinen flex-laatikko) */}
        <div className="ui-panel" style={{ padding: '16px', boxShadow: 'none', display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          <div style={{ flex: 1 }}>
            {mostExpensive.length > 0 ? mostExpensive.map((item, idx) => <PriceRow key={idx} item={item} isExpensive={true} />) : <p className="text-muted">Ei tulevia tunteja.</p>}
          </div>
        </div>
